...
interface BodabodaHeaderProps {
  onToggleSidebar: () => void;
  isSidebarCollapsed: boolean;
}

const BodabodaHeader: React.FC<BodabodaHeaderProps> = ({ onToggleSidebar, isSidebarCollapsed }) => {
    ...
    <header className="fixed top-0 left-0 right-0 bg-white dark:bg-gray-800 z-[60] h-16">
      <div className="flex justify-between items-center h-full">
        <div className="flex items-center h-full">
          ...
          <h1 className="text-xl font-bold text-gray-900 dark:text-white">BodaBoda Smart</h1>
        </div>
      </div>
    </header>
  );
